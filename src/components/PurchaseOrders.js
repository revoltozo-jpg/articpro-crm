...
export default function PurchaseOrders({ detail, setDetail, goDetail, perms }) {
  ...
  useEffect(() => {
    if (detail?.startsWith('new:')) {
      const parts = detail.split(':');
      if (parts[1] === 'vendor') {
        setForm({ vendorId: parts[2], status: 'Draft' });
      } else {
        const soId = parts[1];
        const so = orders.find(o => o.id === soId);
        setForm({ relatedSO: soId, items: so ? so.product + ' x' + so.qty : '', status: 'Draft' });
  ...
  const deleteRecord = async (p) => {
    await deleteDoc(doc(db, 'purchase_orders', p.id));
    if (p.relatedSO) await updateDoc(doc(db, 'orders', p.relatedSO), { vendorPO: '' });
    setDeleteConfirm(null);
    setDetail(null);
    load();
  };
  ...
  const poFields = [
    ...
    { key: 'items', label: 'Items / description', type: 'text' },
    ...(canSeeMoney ? [{ key: 'total', label: 'Total cost to us ($)', type: 'number' }] : []),
    { key: 'orderDate', label: 'Order date', type: 'date' },
    { key: 'expectedDate', label: 'Expected delivery', type: 'date' },
    { key: 'status', label: 'Status', type: 'select', options: statuses.map(s => ({ value: s, label: s })) },
  ];
